...
const createJsonObject = async (endpoint: string, key: string): Promise<unknown> => {
  const mints: any[] = [];
  const tokenMetadata = await getMetadata(new anchor.web3.PublicKey(key), endpoint);
...
export const getMeta = (token: string, endpoint: string) => {
  return createJsonObject(endpoint, token);
};
